...
if(isset($search)) {
	...
	if(isset($_POST['checkedCount']) && $_POST['checkedCount'] == 2){ 
		$query = "SELECT * FROM BankAccounts WHERE AccountNumber CONCAT('%', :thing, '%') ORDER BY DESC");
		echo "2";
	}else{
		$query = "SELECT * FROM BankAccounts WHERE AccountNumber CONCAT('%', :thing, '%') ORDER BY ASC");
		echo "1";
	}
